...
function BootScreen({ onDone }) {
  ...
  useEffect(() => {
    // Solo primera vez por sesión
    if (sessionStorage.getItem("nw-booted")) {
      onDone();
      return;
    }
    ...
    BOOT_LINES.forEach((_, i) => {
      const t = setTimeout(() => {
        ...
        if (i === BOOT_LINES.length - 1) {
          // Esperar un momento y luego pasar al dashboard
          const t2 = setTimeout(() => {
            setFinished(true);
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState(1); // id del primer tab (dashboard)

  const activeTabObj = tabs.find(t => t.id === activeTab) || tabs[0];

  // Cuando termina el boot, activar dashboard
  const handleBootDone = useCallback(() => {
    setBooting(false);
    setActiveTab(tabs[0].id); // asegurar que dashboard es el activo
  }, [tabs]);
  ...
  useEffect(() => {
    ...
    const handler = (e) => {
      if (e.altKey && e.key.toLowerCase() === "t") {
        e.preventDefault(); addTab("powershell");
      } else if (e.altKey && e.key.toLowerCase() === "w") {
        e.preventDefault(); closeTab(activeTab);
  ...
  return (
    <div style={{
      ...
      {/* TabBar — solo cuando no está booteando */}
      {!booting && (
        <TabBar
          tabs={tabs}
          activeTab={activeTab}
  ...
  return (
    <div style={{
            ...
            >
              ...
              {tab.closeable && (
                <span
                  ...
                  style={{ marginLeft: 3, color: "#333", fontSize: "12px", cursor: "pointer", lineHeight: 1 }}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "2px", padding: "0 8px", flexShrink: 0 }}>
        {[
          ...
          <button
            ...
            style={{
              ...
              color: "#333", fontFamily: "var(--font-mono)",
        ...
        {/* Separador */}
        <div style={{ width: 1, height: 16, background: "#1a1a1a", margin: "0 6px" }}/>

        {/* Clock */}
        <span style={{ fontSize: "10px", color: "#FCEE0A", letterSpacing: "0.08em" }}>
          ⏱ {time}
        </span>
      </div>
    </div>
